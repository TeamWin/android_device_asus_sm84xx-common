...
load_module()
{
	...
	if [ -n "$is_module_loaded" ]; then
		...
		insmod "/vendor_dlkm/lib/modules/$1.ko"
		is_module_loaded=$(lsmod | grep "$1")
		if [ -n "$is_module_loaded" ]; then
...
load_module "adsp_loader_dlkm"
load_module "asus_battery_charger_AI2202"
load_module "qti_battery_charger"
load_module "qti_battery_charge_notify"
